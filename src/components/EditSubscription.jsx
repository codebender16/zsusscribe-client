...
class EditSubscription extends React.Component {
  ...
  onFormSubmit = async (event) => {
    ...
    fetch(
      `${process.env.REACT_APP_BACKEND_URL}/subscriptions/${this.props.match.params.id}`,
      {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          subscription: { name, payment_amount, payment_date, category_id },
        }),
      }
    );
  ...
  componentDidMount() {
    ...
    const foundSubscription = this.context.subscriptions.find(
      (subscription) => {
