...
const SignedInNavBar = (props) => {
  ...
  <>
      ...
      <div className={`${ isExpanded ? `block` : `hidden` } justify-end w-full block flex-grow lg:flex lg:items-center lg:w-auto`}>
        <Link className="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4" to="/subscriptions">Subscriptions</Link>
        <Link className="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4" to="/subscriptions/create">Create Subscription</Link>
        <span className="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4" onClick={() => handleLogout(props)}>
        Logout
        </span>
      </div>
